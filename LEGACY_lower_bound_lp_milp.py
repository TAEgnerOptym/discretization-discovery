    ...
    def __init__(self,full_prob,graph_node_2_agg_node,OPT_do_ILP,OPT_use_psi):
        ...
        if self.OPT_do_ilp==0:
            ...
            if debug_on==True:
                init_ineq=None
                init_eq=None
                #if  'init_ineq' in self.full_prob.jy_opt:
                #    init_ineq=self.full_prob.jy_opt['init_ineq']
                #init_eq=self.dict_con_name_2_eq.keys()
                [lp_prob, var_dict, active_ineq, active_eq, active_primal, total_lp_time]=jy_active_set_lp_primal_dual(self.dict_var_name_2_obj,self.dict_var_con_2_lhs_exog,self.dict_con_name_2_LB,self.dict_var_con_2_lhs_eq,self.dict_con_name_2_eq,init_ineq,init_eq)
                #self.full_prob.jy_opt['init_ineq']=active_ineq
                print('total_lp_time='+str(total_lp_time))
                print('time_lp_baseline='+str(self.lp_time))
                print('lp_prob.lp_objective:  ', str(pulp.value(lp_prob.objective)))
                print("self.lp_objective:  "+str(self.lp_objective))
                input('---')
        else:
            self.solve_milp()
