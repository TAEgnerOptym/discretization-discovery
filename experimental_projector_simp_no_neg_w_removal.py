...
class projector:
    ...
    def __init__(self,my_full_solver,h):
        ...
        t1=time.time()
        self.get_non_zero_terms_fg()
        self.time_dict_proj['get_non_zero_terms_fg']=time.time()-t1
        t1=time.time()
    ...
    def get_non_zero_terms_ij_i(self):
        ...
        for ij, P_list in self.hij_2_P_orig.items():
            ...
            if filtered:  # If the filtered list is non-empty:
                if self.MF.jy_opt['doExrtraRemovalEdgesLB']>0.5 and f!=g and  fg not in self.non_zero_fg:
                    my_count=my_count+1
                    continue
    ...
    def make_xpress_LP(self):
       ...
       # input('-in express lp--')
        ...
        print('LP projector start ')
        start_time = time.time()
        lp_prob.solve()
        end_time = time.time()
        print('DONE LP projector ')
