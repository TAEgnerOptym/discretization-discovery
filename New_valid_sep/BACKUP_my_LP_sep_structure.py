...
class my_LP_sep_structure:
    ...
    def __init__(self,my_graph,MF,OPT):
        ...
        var_to_tuples = defaultdict(list)

        for (con, var) in self.con_name_xuv_2_benders_contrib:
            var_to_tuples[var].append((con, var))

        # Step 2: Filter to only those vars that appear more than once
        repeated_vars_with_tuples = {
            var: tuples for var, tuples in var_to_tuples.items() if len(tuples) > 1
        }
        if len(repeated_vars_with_tuples)>0:
            print(repeated_vars_with_tuples)
            input('error ')
    ...
    def make_flow_match(self):
        ...
        for n in self.Nodes:
            ...
            if n in self.nodes_need_pos_slack:
                ...
                #print('self.dict_con_name_2_eq[con_name]')
                #print(self.dict_con_name_2_eq[con_name])
                #print('con_name')
                #print(con_name)
                #input('--')
        ...
        for u in self.my_subset_cust:
            for v in self.my_subset_cust:
                ...
                if var_name in my_primal:
                    ...
                    self.dict_con_name_2_eq[con_name]=my_primal[var_name]+self.OPT['my_magnanti']
                    self.con_name_xuv_2_benders_contrib[tuple([con_name,var_name])]=-1
                    for e in self.uv_2_E[tuple([u,v])]:
                        e_var_name=self.dict_e_2_var_name[e]
                        self.dict_var_con_2_lhs_eq[tuple([e_var_name,con_name])]=1
                    
                    if self.try_slack_match:
                        var_slack_pos='VarPosMatch_'+str(u)+'_'+str(v)
                        var_slack_neg='VarNegMatch_'+str(u)+'_'+str(v)
                        
                        self.dict_var_con_2_lhs_eq[tuple([var_slack_pos,con_name])]=1
                        self.dict_var_con_2_lhs_eq[tuple([var_slack_neg,con_name])]=-1
    ...
    def make_benders_cut(self):
        ...
        for con_name in self.con_name_2_benders_OBJ:
            ...
            benders_obj+=self.con_name_2_benders_OBJ[con_name]*dual_val
        for con_name in self.lp_dual_solution:
            dual_val=self.lp_dual_solution[con_name]
            if abs(dual_val)>1.1:
                con_name_matches = {
                    v: val
                    for (v, c), val in self.dict_var_con_2_lhs_eq.items()
                    if c == con_name
                }
                print('con_name_matches')
                print(con_name_matches)
                print('con_name')
                print(con_name)
                print('dual_val')
                print(dual_val)
                input('ok wierd')
            #    print('self.con_name_2_benders_OBJ[con_name]')
            #    print(self.con_name_2_benders_OBJ[con_name])
            #    print('dual_val')
            #    print(dual_val)
        ...
        #input('- done cut obje--')
        #for con_name in self.lp_dual_solution:
        #    if abs(self.lp_dual_solution[con_name])>0.0001:
        #        print('con_name')
        #        print(con_name)
        #        print('self.lp_dual_solution[con_name]')
        #        print(self.lp_dual_solution[con_name])
        #input('looking at stuff')
        for (con_name,var_name) in self.con_name_xuv_2_benders_contrib:
            dual_val=self.lp_dual_solution[con_name]
            if var_name not in benders_dict:
                benders_dict[var_name]=0
            else:
                input('this is present already should i be concerned i think so ')
            ...
            if abs(tmp)>1.1:
                ...
                print('dual_val')
                print(dual_val)
                print('self.con_name_xuv_2_benders_contrib[(con_name,var_name)]')
                print(self.con_name_xuv_2_benders_contrib[(con_name,var_name)])
                print('con_name')
                print(con_name)
                print('var_name')
                print(var_name)
                input('how does that make sense ')
            benders_dict[var_name]+=tmp
            if abs(benders_dict[var_name])>1.1:
                print('tmp')
                print(tmp)
                print('benders_dict[var_name]')
                print(benders_dict[var_name])
                input('Also weird how does that make sense ')
    ...
    def add_ineq_to_MF(self):
        ...
        for primal_var in self.benders_dict:
            ...
            if 1>0 or abs(val)>0.0001 or self.primal_sol[primal_var]>0.0001:
