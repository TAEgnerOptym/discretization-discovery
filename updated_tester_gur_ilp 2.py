...
def branchCallback(model, where):
    if where == gp.GRB.Callback.MIPSOL:
        ...
        if status == GRB.OPTIMAL:
            ...
            # Get selected branching variable info where possible
            try:
                # Use alternative approach to get branching info
                # Print variable values at this node to understand branching candidates
                if node_count % 10 == 0:  # Only for every 10th node to avoid too much output
                    print(f"Top variable values at node {node_count}:")
                    vars = model.getVars()
                    # Get node solution if possible
                    try:
                        nodeSol = model.cbGetNodeRel()
                        # Print some of the fractional binary variables
                        count = 0
                        ...
                        print("  Could not get node solution")
            except:
